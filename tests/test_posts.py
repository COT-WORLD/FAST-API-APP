...
import pytest
from Fastpost.schemas import PostOut, PostSchema
...
def test_unauthorised_user_create_post(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "Awesome new title",
                      "content": "Awesome new content", "published": True})
    assert res.status_code == 401
...
def test_update_post(authorised_client, test_posts, test_user):
    res = authorised_client.put(f"/posts/{test_posts[0].id}", json={
                                "title": "Awesome new title", "content": "Awesome new content", "published": True})
...
def test_unauthorised_user_update_post(client, test_posts):
    res = client.put(f"/posts/{test_posts[0].id}", json={
                     "title": "Updated title", "content": "Updated content", "published": True})
    assert res.status_code == 401


def test_update_other_user_post(authorised_client, test_posts):
    res = authorised_client.put(f"/posts/{test_posts[3].id}", json={
                                "title": "Updated title", "content": "Updated content", "published": True})
    assert res.status_code == 401
...
def test_update_post_not_exist(authorised_client, test_posts):
    res = authorised_client.put(
        f"/posts/88888", json={"title": "Updated title", "content": "Updated content", "published": True})
    assert res.status_code == 404
