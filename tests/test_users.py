...
from Fastpost.schemas import Token, UserSchema
...
def test_root(client: TestClient):
    ...
    assert (
        ...
        == "Hello World successfully deployed on Render via CICD pipeline(Github Actions)"
    )


def test_create_user(session: Session, client: TestClient):
    res = client.post(
        "/users/", json={"email": "john@gmail.com", "password": "password123"}
    )
...
def test_login_user_token_check(client: TestClient, test_user):
    res = client.post(
        "/login", data={"username": "james@gmail.com", "password": "password123"}
    )
